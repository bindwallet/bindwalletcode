...
function testVector (description, wordlist, password, v, i) {
  ...
  var vmnemonic = v[1]
  var vseedHex = v[2]
  var joinchar = description === 'Japanese' ? '\u3000' : ' '
  ...
  test('for ' + description + '(' + i + '), ' + ventropy, function (t) {
    ...
    t.equal(bip39.entropyToMnemonic(ventropy, wordlist, joinchar), vmnemonic, 'entropyToMnemonic returns ' + vmnemonic.slice(0, 40) + '...')

    function rng () { return Buffer.from(ventropy, 'hex') }
    t.equal(bip39.generateMnemonic(undefined, rng, wordlist, joinchar), vmnemonic, 'generateMnemonic returns RNG entropy unmodified')
